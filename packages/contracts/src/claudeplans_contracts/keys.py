...
def validate_key_segment(value: str) -> str:
    ...
    Segments compose the opaque storage key `owner/project/type/slug`; one that is
...
def document_key(owner_id: str, project: str, doc_type: DocType, slug: str) -> str:
    ...
    `doc_type` is a DocType enum, so it needs no segment validation.
    """
    validate_key_segment(owner_id)
    validate_key_segment(project)
    validate_key_segment(slug)
    # DocType is a StrEnum, so the f-string yields its value ("plan"/"research").
    return f"{owner_id}/{project}/{doc_type}/{slug}"


def key_for_document(doc: Document) -> str:
    """Storage key for an already-built Document."""
    return document_key(doc.owner_id, doc.project, doc.type, doc.slug)
