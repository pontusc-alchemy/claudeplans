...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --locked --no-dev --no-editable --package claudeplans
...
# floor, and the shared uv cache mount makes the overlapping resolution near-instant. packages/
# is COPYed so uv can discover the workspace during resolution; it is never imported from here.
FROM base AS ci
COPY pyproject.toml uv.lock ./
COPY packages/ ./packages/
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --locked --all-packages --no-install-workspace
ENV PATH=/opt/venv/bin:$PATH \
