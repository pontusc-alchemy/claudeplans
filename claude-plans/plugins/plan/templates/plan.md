---
title:
description:
type: plan
status: draft
date:
tag:
---

<!-- One-paragraph intro: what this plan executes; link to its research doc via a relative .md link. -->

## Executive summary

<!-- Answer the user's explicit questions, then a phase/status pipe table:
     | Phase | Anchor | Pill | Note |. Link each phase via [anchor](#phase-1). -->

## Phase 1 — <name> {#phase-1}

<span class="pill gap">gap</span>

<!-- Exact, ordered work with commands, paths, and version pins. -->

- [ ] First task
- [ ] Second task

**Exit criteria**

<!-- The bar for sign-off before the next phase opens. -->

- Criterion one met

## Gaps & decisions {#gaps}

<!-- Cross-cutting open decisions; pill each (gap/partial/ok). -->

- [ ] <span class="pill gap">gap</span> Decision still open

## Learnings {#learnings}

<!-- Appended during /plan:iterate as discoveries happen. Format: -->
<!-- - **YYYY-MM-DD (phase N)** — what we learned, what changed because of it. -->

## Review log {#review-log}

<!-- Appended by /plan:review: date · what was verified · what changed · what remains open. -->
