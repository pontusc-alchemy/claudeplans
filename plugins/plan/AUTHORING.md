...
### Cheapest read for a question

One verb each. Reaching for a second call usually means the wrong row was picked.

| Question | Verb | Notes |
|---|---|---|
| What is in this project? | `doc list <project>` | slug, title, type, status, rev, updated_at, primary_research_ref per doc |
| Which doc has which parent? | `project lineage <project>` | the whole forest in one call; or read `primary_research_ref` off `doc list` |
| What changed since I looked? | `doc list <project> --since <iso>` | compares `updated_at` as text; a doc with no timestamp is dropped |
| What hangs off this doc? | `doc list <project> --parent <slug>` | direct children only — `project lineage` gives the nesting |
| What is this doc's current rev? | `doc rev <project> <slug>` | bare token, composes into `--rev`; `doc list` also carries every rev |
| Where is a phase or task index? | `doc phases <project> <slug>` | leads with the rev, so it feeds a position-sensitive write directly |
| Where did I write about X? | `search <project> <query>` | titles and headings only, not bodies |
| What does the CLI accept? | `schema` | offline; enums, exit codes, envelopes, per-command flags |

`--type`, `--status`, `--parent` and `--since` all filter one `doc list` call
client-side, so combining them costs the same single request as listing everything.
