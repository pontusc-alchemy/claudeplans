...
claudeplans doc create|create-many|get|delete|status|set-status|set|phases|link|unlink|list|rev|view
...
Exit codes: `0` ok · `2` usage error (bad flag/arg — reserved, never a domain error) · `3` forbidden · `4` validation · `5` not-found · `6` transport (service unreachable / bad `--url`) · `7` partial (`create-many` only) · `9` stale-rev. Every nonzero code except `7` means **nothing landed**; `7` means a prefix of the array landed and `ops[]` names exactly which, so resuming is a tail. Every domain error also prints a structured `{"error":<kind>,…}` line to stderr (never a stacktrace); stale-rev carries `current_rev`, plus `conflict:"exists"` when a create lost to an existing slug — that one is not retryable, pick a different slug. A malformed `--rev` (not a decimal integer string) is rejected as `invalid_rev` (exit 4) before any concurrency check — do not retry it; fix the value instead.
